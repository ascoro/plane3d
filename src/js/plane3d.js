...
coroSite.Terrain = function(settings){
	...
	thiz.getMesh = function(){
		...
		texture = new THREE.Texture( generateTexture( heightData, width, depth ), new THREE.UVMapping(), THREE.ClampToEdgeWrapping, THREE.ClampToEdgeWrapping );
...
var plane3d = function(settings){
	...
	var terrain = new coroSite.Terrain({width:worldWidth,depth:worldDepth});
	...
	var colisionDetector = function(){
		...
		var yMap = geometry.vertices[Math.floor(xMap+zMap*256)].y;
		var yCamera = camera.position.y;
		if(yMap){
			if(yCamera-yMap>0 != isAbove){
				isAbove=!isAbove;
				console.log("Is above: "+isAbove);
			}
...
$(document).on('click',function(){new plane3d();});
